...
const page = async({params}: {params: Promise<{ vid: string, profile:string }>}) => {
    const session = await auth()
    const newParams = await params;
    const response = await getVideoDetail(newParams?.vid)
   
  return (
    <Suspense fallback={<div className=' flex justify-center bg-transparent p-5 items-center w-full'><CLoader/></div>}>
