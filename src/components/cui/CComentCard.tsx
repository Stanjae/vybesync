...
import {
  deleteReplyByKey,
  deleteVideoComment,
  fetchCommentLikeArray,
  getCommentsWithLikedUserIds,
  handleCommentLikes,
  handleReplyLikes,
  insertReply,
} from "@/lib/actions";
...
export const CComentCard = ({
  ...
  const { mutateAsync: addAsync, isPending: Pending } = useMutation({
  ...
  return (
    <Card
      className={` ${isPending ? "opacity-5" : "opacity-100"} p-2 bg-light-muted border-none`}
    >
      <motion.section
        initial={"hidden"}
        variants={small}
      ...
      >
          ...
          <div className=" text-background flex gap-8 items-center">
            ...
            {userId == item?.userId?._id && (
              <CommentDialog
                ...
                description="Do you wanna Delete Permanently?"
                trigger={
                  <Button
                    size={"sm"}
                    className="focus-visible:ring-0 focus-within:ring-0 outline-none text-primary-custom"
                  >
                    Clear
                  </Button>
                }
        ...
        <div className=" ml-auto">
          <Button onClick={toggleLike} className=" rounded-full" size={"icon"}>
            <Heart
              className={`${likeArray?.includes(userId as string) ? "text-primary-custom" : ""}`}
            />
          </Button>
          <p className=" text-base text-muted-custom-text text-center">
            {millify((likeArray?.length as number) || 0)}
          </p>
        </div>
      </motion.section>
    </Card>
      ...
      >
        ...
        <div className=" space-y-1">
          ...
          <div className=" text-background flex gap-10 items-center">
            <span className=" text-sm">{created}</span>
            <Button className=" text-background" variant={"link"} size={"sm"}>
              Reply
            </Button>
          </div>
          <Button
...
export const ReplyComentCard = ({
  ...
  useEffect(() => {
    const getLikesArr = async () => {
      const response = await getCommentsWithLikedUserIds(
        commentId,
        itemo?._key as string
      );
      setLikeArray(response);
    };
    getLikesArr();
  ...
  return (
    <motion.div
      ...
      variants={small}
      animate={"visible"}
      className={`${isPending ? "opacity-5" : ""} p-2 space-y-2 bg-light-muted border-none`}
    >
      ...
      <section className=" gap-2 flex items-start">
        ...
        <div className=" ml-auto">
          ...
          <p className=" text-sm text-muted-custom-text text-center">
            {millify((likeArray?.length as number) || 0)}
          </p>
        </div>
      </section>
