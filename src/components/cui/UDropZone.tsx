...
export const navs = [
  {
    ...
    icon: <VideoIcon className=" size-5 text-foreground" />,
  },
  {
    title: "File formats",
    text: "Recommended: .mp4. Other major formats are supported.",
    icon: <FileVideo className="size-5 text-foreground" />,
  },
  {
    title: "Video resolutions",
    text: "High-resolution recommended: 1080p, 1440p, 4K.",
    icon: <FileTerminal className="size-5 text-foreground" />,
  },
  {
    title: "Aspect ratios",
...
export function UDropzone({ session }: { session: Session }) {
      ...
      await axios
        .post("/api/upload", formData, {
          onUploadProgress: (progressEvent) => {
            ...
            console.log(
              `Upload Progress: ${progressEvent.total} || ${progress.toFixed(2)}%`
            );
          },
        })
        .then((response) => {
  ...
  const handleRemove = async () => {
    try {
      const response = await axios.delete("/api/upload/remove", {
        data: { public_id: videoURL?.public_id },
      });
      setVideoURL(null);
  ...
  return (
    <main>
      {!step ? (
        <div className="w-full max-w-5xl p-4 mx-auto min-h-96 border border-dashed bg-background border-neutral-200 rounded-lg">
          {unfinished && (
            <div className=" my-2 rounded-lg text-foreground flex items-center justify-between bg-[#F8F8F8] py-3 px-2">
              <div className=" flex gap-2 items-center">
                <AlertCircle className=" text-red-600 size-5" />A video you were
                editing wasn&apos;t saved. Continue Editing
              </div>
              <div className=" flex gap-2 items-center">
                <Button
                  onClick={handleRemove}
                  className=" bg-primary-custom cursor-pointer hover:bg-primary-custom/80 text-background"
                >
                  Discard
                </Button>
                <Button
        ...
        <UploadForm
          ...
          handleSuccess={clearPostAfterSuccess}
          session={session}
          videoUrl={videoURL}
        />
      )}
