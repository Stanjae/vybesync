...
const OthersSearchedFor = async({query}:{query:string}) => {
  ...
  return (
    <div className=' space-y-2'>
       <h6 className=' text-sm font-medium text-muted-custom-text'>Others Searched For</h6>
            
            <ul className=' space-y-2'>
                {searches?.map((item, index) =>(
                    <Link key={index} className="block hover:bg-muted-custom p-1 rounded-md" href={`search?query=${item?.title.toLowerCase()}`}>
